...
async function main() {
    ...
    // --- Status Tool ---
    server.tool("nelly_instructions", "Check Nelly's status — platform, sessions directory, and whether session data exists. Useful for diagnostics. Instructions are already loaded via MCP init.", {}, async () => {
    ...
    // --- Search Tool ---
    server.tool("nelly_search", "Search past Claude Code conversations. Nelly searches through all session transcripts for matching keywords and returns sessions with snippets and resume commands. Use multiple searches with different terms for best results (spiral search pattern).", {
    ...
    // --- Context Tool ---
    server.tool("nelly_context", "Get expanded context from a specific session. Use this after nelly_search when the user wants to see more detail from a particular session before resuming it.", {
    ...
    server.tool("nelly_recent", "List the most recent Claude Code sessions. Useful when the user doesn't remember specific keywords but knows it was recent.", {
