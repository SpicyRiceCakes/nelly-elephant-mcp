...
// ── Send ──────────────────────────────────────
export async function sendIMessage(text, handle) {
  ...
  `;
    try {
        ...
        if (error.message?.includes("assistive") ||
            error.message?.includes("automation")) {
            throw new Error("Automation permission denied. Grant Terminal/Claude access in System Preferences > Privacy & Security > Automation.");
        }
...
export async function searchIMessages(options) {
    ...
    const conditions = ["m.text IS NOT NULL", "length(m.text) > 0"];
    if (query) {
  ...
  `;
    try {
        ...
        if (error.message?.includes("unable to open")) {
            throw new Error("Cannot access Messages database. Grant Full Disk Access to Terminal in System Preferences > Privacy & Security.");
        }
        throw new Error(`Database query failed: ${error.message}`);
    }
}
// ── Conversations ─────────────────────────────
export async function listIMessageConversations(limit = 20) {
    const sql = `
    SELECT
      h.id as handle,
